...
public class ItemRenderer
{
    ...
    public void renderItem(EntityLivingBase entityIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform)
    {
        if (heldStack != null)
        {
            ...
            if (this.itemRenderer.shouldRenderItemIn3D(heldStack))
            {
                ...
                Animations animations = UwU.INSTANCE.getModuleManager().getModule(Animations.class);
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            GlStateManager.pushMatrix();
            KillAura aura = UwU.INSTANCE.getModuleManager()
    ...
    {
        if(UwU.INSTANCE.getModuleManager().getModule(Camera.class).isEnabled() && UwU.INSTANCE.getModuleManager().getModule(Camera.class).setting.isEnabled("No Fire"))
            return;
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
