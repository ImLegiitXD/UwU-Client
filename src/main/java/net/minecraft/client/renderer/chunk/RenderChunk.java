...
public class RenderChunk
{
    ...
    public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator)
    {
        ...
        HashSet<TileEntity> setTileEntities = Sets.newHashSetWithExpectedSize(32);
        ...
        for (int iy = blockpos.getY(); iy <= blockpos1.getY(); ++iy) {
            for (int iz = blockpos.getZ(); iz <= blockpos1.getZ(); ++iz) {
                for (int ix = blockpos.getX(); ix <= blockpos1.getX(); ++ix) {
                    blockposm.set(ix, iy, iz);
                        ...
                        if (tileentity != null) {
                            ...
                            if (tileentityspecialrenderer != null) {
                                ...
                                if (tileentityspecialrenderer.forceTileEntityRender()) {
                                    setTileEntities.add(tileentity);
                                }
                            }
                        }
                    }

                    EnumWorldBlockLayer[] aenumworldblocklayer = flag ? ENUM_WORLD_BLOCK_LAYERS : this.blockLayersSingle;
                    if (!flag) {
                        aenumworldblocklayer[0] = block.getBlockLayer();
                    }

                    for (EnumWorldBlockLayer enumworldblocklayer : aenumworldblocklayer) {
                        if (flag && !Reflector.callBoolean(block, Reflector.ForgeBlock_canRenderInLayer, enumworldblocklayer)) {
                            continue;
                        }
                        if (flag1) {
                        ...
                        EnumWorldBlockLayer fixedLayer = this.fixBlockLayer(iblockstate, enumworldblocklayer);
                        int k = fixedLayer.ordinal();

                        if (block.getRenderType() != -1) {
                            WorldRenderer worldrenderer = generator.getRegionRenderCacheBuilder().getWorldRendererByLayerId(k);
                            worldrenderer.setBlockLayer(fixedLayer);
                            ...
                            if (!compiledchunk.isLayerStarted(fixedLayer)) {
                                compiledchunk.setLayerStarted(fixedLayer);
        ...
        try
        {
            Set<TileEntity> set = Sets.newHashSet(setTileEntities);
            Set<TileEntity> set1 = Sets.newHashSet(this.setTileEntities);
            set.removeAll(this.setTileEntities);
            set1.removeAll(setTileEntities);
            this.setTileEntities.clear();
            this.setTileEntities.addAll(setTileEntities);
    ...
    public Chunk getChunk()
    {
        ...
        this.chunk = this.world.getChunkFromBlockCoords(this.position);
        return this.chunk;
    }

    private Chunk getChunk(BlockPos p_getChunk_1_)
    {
        // This method is now redundant, getChunk() handles caching.
        // Kept for binary compatibility if other mods call it.
    ...
    public void setRenderChunkNeighbour(EnumFacing p_setRenderChunkNeighbour_1_, RenderChunk p_setRenderChunkNeighbour_2_)
    {
        ...
        // When a neighbour is set, our valid neighbour cache is now dirty
        this.renderChunkNeighboursUpated = false;
    }
    ...
    private void updateRenderChunkNeighboursValid()
    {
        ...
        RenderChunk north = this.renderChunkNeighbours[EnumFacing.NORTH.ordinal()];
        RenderChunk south = this.renderChunkNeighbours[EnumFacing.SOUTH.ordinal()];
        RenderChunk west = this.renderChunkNeighbours[EnumFacing.WEST.ordinal()];
        RenderChunk east = this.renderChunkNeighbours[EnumFacing.EAST.ordinal()];

        this.renderChunkNeighboursValid[EnumFacing.NORTH.ordinal()] = (north != null && north.getPosition().getZ() == j - 16) ? north : null;
        this.renderChunkNeighboursValid[EnumFacing.SOUTH.ordinal()] = (south != null && south.getPosition().getZ() == j + 16) ? south : null;
        this.renderChunkNeighboursValid[EnumFacing.WEST.ordinal()] = (west != null && west.getPosition().getX() == i - 16) ? west : null;
        this.renderChunkNeighboursValid[EnumFacing.EAST.ordinal()] = (east != null && east.getPosition().getX() == i + 16) ? east : null;
