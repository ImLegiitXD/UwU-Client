...
import wtf.uwu.features.modules.impl.visual.Shaders;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        UwU.INSTANCE.getModuleManager().getModule(Shaders.class).renderShaders();
        UwU.INSTANCE.getEventManager().call(new Render2DEvent(partialTicks, scaledresolution));
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer entityplayer)
        {
            ...
            float f = this.zLevel;
            this.zLevel = -90.0F;
            if(UwU.INSTANCE.getModuleManager().getModule(Interface.class).isEnabled() && UwU.INSTANCE.getModuleManager().getModule(Interface.class).hotBar.get()){
                RoundedUtils.drawRound(i - 91, sr.getScaledHeight() - 22, 182, 22,6 ,new Color(0,0,0,100));
                RoundedUtils.drawRoundOutline(i - 91 - 1 + SpoofSlotUtils.getSpoofedSlot() * 20, sr.getScaledHeight() - 22 - 1,24,22,6,0.5f,new Color(0,0,0,0),new Color(UwU.INSTANCE.getModuleManager().getModule(Interface.class).color(),true));
            } else {
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        if (UwU.INSTANCE.getModuleManager().getModule(Interface.class).isEnabled() && UwU.INSTANCE.getModuleManager().getModule(Interface.class).customScoreboard.get()) {
            UwU.INSTANCE.getModuleManager().getModule(Interface.class).drawScoreboard(scaledRes,objective,objective.getScoreboard(),objective.getScoreboard().getSortedScores(objective));
            return;
        }

        if (UwU.INSTANCE.getModuleManager().getModule(Interface.class).isEnabled() && UwU.INSTANCE.getModuleManager().getModule(Interface.class).hideScoreboard.get())
            return;
