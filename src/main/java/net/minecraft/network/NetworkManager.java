...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
    ...
    public void sendPacket(Packet packetIn) {
        ...
        if (direction == CLIENTBOUND) UwU.INSTANCE.getEventManager().call(event);
    ...
    protected void messageReceived(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            ...
            try {
                if (UwU.INSTANCE.getModuleManager().getModule(Disabler.class).isEnabled() && UwU.INSTANCE.getModuleManager().getModule(Disabler.class).options.isEnabled("GrimAC") && UwU.INSTANCE.getModuleManager().getModule(Disabler.class).grim.isEnabled("Post") && UwU.INSTANCE.getModuleManager().getModule(Disabler.class).getPost() && UwU.INSTANCE.getModuleManager().getModule(Disabler.class).postDelay(p)) {
                    Minecraft.getMinecraft().addScheduledTask(() -> {
                        UwU.INSTANCE.getModuleManager().getModule(Disabler.class).getStoredPackets().add(p);
                    });
                } else {
