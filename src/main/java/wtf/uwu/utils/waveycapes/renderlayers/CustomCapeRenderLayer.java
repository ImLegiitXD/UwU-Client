 ...
 */
package wtf.uwu.utils.waveycapes.renderlayers;

import net.minecraft.util.ResourceLocation;
import wtf.uwu.UwU;
import wtf.uwu.features.modules.impl.visual.Interface;
import wtf.uwu.utils.render.shader.ShaderUtils;
import wtf.uwu.utils.waveycapes.CapeMovement;
import wtf.uwu.utils.waveycapes.CapeStyle;
import wtf.uwu.utils.waveycapes.WindMode;
import wtf.uwu.utils.waveycapes.config.Config;
import wtf.uwu.utils.waveycapes.sim.StickSimulation;
import wtf.uwu.utils.waveycapes.util.Mth;
...
public class CustomCapeRenderLayer implements LayerRenderer<AbstractClientPlayer> {
    ...
    public void doRenderLayer(AbstractClientPlayer abstractClientPlayer, float paramFloat1, float paramFloat2, float deltaTick,
        ...
        if (!UwU.INSTANCE.getModuleManager().getModule(Interface.class).wavey.get())
            return;

        if (!abstractClientPlayer.isInvisible() && (abstractClientPlayer.hasPlayerInfo() && abstractClientPlayer.isWearing(EnumPlayerModelParts.CAPE) && abstractClientPlayer.getLocationCape() != null || abstractClientPlayer.getName().equals(Minecraft.getMinecraft().getSession().getUsername()) && UwU.INSTANCE.getModuleManager().getModule(Interface.class).isEnabled() && UwU.INSTANCE.getModuleManager().getModule(Interface.class).cape.get())) {
            if (Config.capeMovement == CapeMovement.BASIC_SIMULATION) {
                abstractClientPlayer.updateSimulation(abstractClientPlayer, partCount);
            }

            if (abstractClientPlayer == Minecraft.getMinecraft().thePlayer && UwU.INSTANCE.getModuleManager().getModule(Interface.class).cape.get()) {
                this.playerRenderer.bindTexture(new ResourceLocation("uwu client/texture/cape/overlay.png"));
            } else {
            ...
            if (Config.capeStyle == CapeStyle.SMOOTH) {
                if (abstractClientPlayer == Minecraft.getMinecraft().thePlayer) {
                    if (UwU.INSTANCE.getModuleManager().getModule(Interface.class).cape.get()) {
