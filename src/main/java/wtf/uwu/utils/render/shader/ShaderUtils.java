package wtf.uwu.utils.render.shader;
...
public class ShaderUtils implements InstanceAccess {
    ...
    private static final String DEFAULT_VERTEX_SHADER_RESOURCE = "uwu client/shader/vertex.vsh";

    private static final Map<String, String> PREDEFINED_FRAGMENT_SHADERS = new HashMap<>();

    private static final String S_BLOOM = """
            #version 120
    ...
    private static final String S_ROUND_RECT_TEXTURE = """
            #version 120
    ...
    private static final String S_ROUND_RECT_OUTLINE = """
            #version 120
            uniform vec2 location, rectSize;
    ...
    private static final String S_ROUNDED_RECT_GRADIENT = """
            #version 120
            uniform vec2 location, rectSize;
    ...
    private static final String S_ROUNDED_RECT = """
            #version 120
            uniform vec2 location, rectSize;
            uniform vec4 color;
            uniform float radius;
    ...
    private static final String S_KAWASE_UP_BLOOM = """
            #version 120
            uniform sampler2D inTexture, textureToCheck;
    ...
    private static final String S_KAWASE_DOWN_BLOOM = """
            #version 120
    ...
    private static final String S_KAWASE_UP = """
            #version 120
            uniform sampler2D inTexture, textureToCheck;
            uniform vec2 halfpixel, offset, iResolution;
            uniform int check;
    ...
    private static final String S_KAWASE_DOWN = """
            #version 120
            uniform sampler2D inTexture;
            uniform vec2 offset, halfpixel, iResolution;
            void main() {
    ...
    private static final String S_GRADIENT = """
            #version 120
            uniform vec2 location, rectSize;
    ...
    private static final String S_MAINMENU = """
            uniform float TIME;
    ...
    private static final String S_GAUSSIAN_BLUR = """
            #version 120
            uniform sampler2D textureIn;
            uniform vec2 texelSize, direction;
            uniform float radius;
    ...
    private static final String S_CAPE = """
    ...
    private static final String S_GLOW = """
            #version 120
    ...
    private static final String S_OUTLINE = """
            #version 120
            uniform vec2 texelSize, direction;
    ...
    static {
        PREDEFINED_FRAGMENT_SHADERS.put("shadow", S_BLOOM);
        PREDEFINED_FRAGMENT_SHADERS.put("roundRectTexture", S_ROUND_RECT_TEXTURE);
        PREDEFINED_FRAGMENT_SHADERS.put("roundRectOutline", S_ROUND_RECT_OUTLINE);
        PREDEFINED_FRAGMENT_SHADERS.put("roundedRect", S_ROUNDED_RECT);
        PREDEFINED_FRAGMENT_SHADERS.put("roundedRectGradient", S_ROUNDED_RECT_GRADIENT);
        PREDEFINED_FRAGMENT_SHADERS.put("gradient", S_GRADIENT);
        PREDEFINED_FRAGMENT_SHADERS.put("mainmenu", S_MAINMENU);
        PREDEFINED_FRAGMENT_SHADERS.put("kawaseUp", S_KAWASE_UP);
        PREDEFINED_FRAGMENT_SHADERS.put("kawaseDown", S_KAWASE_DOWN);
        PREDEFINED_FRAGMENT_SHADERS.put("kawaseUpBloom", S_KAWASE_UP_BLOOM);
        PREDEFINED_FRAGMENT_SHADERS.put("kawaseDownBloom", S_KAWASE_DOWN_BLOOM);
        PREDEFINED_FRAGMENT_SHADERS.put("gaussianBlur", S_GAUSSIAN_BLUR);
        PREDEFINED_FRAGMENT_SHADERS.put("cape", S_CAPE);
        PREDEFINED_FRAGMENT_SHADERS.put("outline", S_OUTLINE);
        PREDEFINED_FRAGMENT_SHADERS.put("glow", S_GLOW);
    }
    ...
    public ShaderUtils(String fragmentShaderLoc, String vertexShaderLoc) {
        ...
        try {
            ...
            String predefinedSource = PREDEFINED_FRAGMENT_SHADERS.get(fragmentShaderLoc);

            if (predefinedSource != null) {
                fragmentShaderStream = new ByteArrayInputStream(predefinedSource.getBytes());
            } else {
                fragmentShaderStream = mc.getResourceManager().getResource(new ResourceLocation(fragmentShaderLoc)).getInputStream();
            }
            int fragmentShaderID = createShader(fragmentShaderStream, GL_FRAGMENT_SHADER);
            glAttachShader(program, fragmentShaderID);

            InputStream vertexShaderStream = mc.getResourceManager().getResource(new ResourceLocation(vertexShaderLoc)).getInputStream();
            int vertexShaderID = createShader(vertexShaderStream, GL_VERTEX_SHADER);
            glAttachShader(program, vertexShaderID);

        } catch (IOException e) {
            System.err.println("Failed to load shader resources: " + fragmentShaderLoc + ", " + vertexShaderLoc);
            e.printStackTrace();
        }
        ...
        if (status == 0) {
            ...
            throw new IllegalStateException("Shader program linking failed! Log: " + errorLog);
        }
        this.programID = program;
    }

    public ShaderUtils(String fragmentShaderLoc) { // Constructor changed back
        this(fragmentShaderLoc, DEFAULT_VERTEX_SHADER_RESOURCE);
    }


    public void init() { // Method names reverted to original for consistency if preferred
        glUseProgram(programID);
    }

    public void unload() { // Method names reverted
        glUseProgram(0);
    }

    public int getUniform(String name) { // Method names reverted
    ...
    public void setUniformf(String name, float... args) {
        ...
        switch (args.length) {
            ...
            case 4:
                ...
                break;
            default:
                System.err.println("Unsupported number of float arguments for uniform: " + name);
        }
    }
        ...
        if (args.length > 1) {
            switch (args.length) {
                ...
                case 4:
                    ...
                    break;
                default:
                    System.err.println("Unsupported number of int arguments for uniform (2-4 expected): " + name);
                    break;
            }
        } else if (args.length == 1) {
            glUniform1i(loc, args[0]);
        } else {
            System.err.println("Unsupported number of int arguments for uniform (1-4 expected): " + name);
        }
    }
    ...
    public static void drawQuads(float width, float height) { // Kept this overload consistent
    ...
    private int createShader(InputStream inputStream, int shaderType) {
        ...
        if (glGetShaderi(shader, GL_COMPILE_STATUS) == 0) {
            String errorLog = glGetShaderInfoLog(shader, 4096);
            glDeleteShader(shader);
            String typeString = shaderType == GL_VERTEX_SHADER ? "Vertex" : (shaderType == GL_FRAGMENT_SHADER ? "Fragment" : "Unknown");
            System.err.println(glGetShaderInfoLog(shader, 4096));
            throw new IllegalStateException(String.format("Shader (%s type) failed to compile! Log: %s", typeString, errorLog));
        }
        return shader;
    }
    ...
    public static String readInputStream(InputStream inputStream) {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream))) {
            ...
            System.err.println("Failed to read InputStream to String.");
            e.printStackTrace();
            return "";
        }
