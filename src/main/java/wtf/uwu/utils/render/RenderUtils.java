...
public class RenderUtils implements InstanceAccess {
    ...
    public static void renderEnchantText(ItemStack stack, double x, double y, float scale) {
        ...
        if (stack.getRarity() == EnumRarity.EPIC) {
            ...
            mc.fontRendererObj.drawString(net.minecraft.util.EnumChatFormatting.BOLD + "God", (float) (x), (float) height, new Color(255, 255, 0).getRGB());
    ...
    private static void drawEnchantTag(String text, double x, double y, float scale) {
        ...
        GL11.glTranslated(x, y, x);
        GL11.glScaled(scale, scale, scale);
        mc.fontRendererObj.drawStringWithShadow(text, (float) 0, (float) 0, -1);
        GlStateManager.enableDepth();
        GlStateManager.popMatrix();
    }
    ...
    public static void drawExhiRect(float x, float y, float x2, float y2, float alpha) {
        ...
        Gui.drawRect((int)(x - 3F), (int)(y - 3F), (int)(x2 + 3F), (int)(y2 + 3F), new Color(50, 50, 50, (int)alpha).getRGB());
        Gui.drawRect((int)(x - 2.5F), (int)(y - 2.5F), (int)(x2 + 2.5F), (int)(y2 + 2.5F), new Color(26, 26, 26, (int)alpha).getRGB());
        Gui.drawRect((int)(x - 0.5F), (int)(y - 0.5F), (int)(x2 + 0.5F), (int)(y2 + 0.5F), new Color(50, 50, 50, (int)alpha).getRGB());
        Gui.drawRect((int)x, (int)y, (int)x2, (int)y2, new Color(18, 18, 18, (int)alpha).getRGB());
    }
        ...
        try {
            for (final Vec3 v : vec3s) {
                ...
                if (draw) {
                    ...
                    final Color c = new Color(UwU.INSTANCE.getModuleManager().getModule(Interface.class).color(0));
    ...
    private static final Deque<Rectangle> scissorStack = new ArrayDeque<>();

    public static void pushScissor(float x, float y, float width, float height) {
        Rectangle scissor = new Rectangle((int) x, (int) y, (int) Math.ceil(width), (int) Math.ceil(height));

        if (!scissorStack.isEmpty()) {
            Rectangle parent = scissorStack.peek();
            scissor = scissor.intersection(parent);
        }
    ...
    private static void applyScissor(Rectangle scissor) {
        ScaledResolution sr = new ScaledResolution(mc);
        int scale = sr.getScaleFactor();

        int transY = sr.getScaledHeight() - scissor.y - scissor.height;
    ...
    public static void scale(float scaleX, float scaleY, float centerX, float centerY, Runnable block) {
        glPushMatrix();
    ...
    public static void startScissor(float x, float y, float width, float height) {
        ...
        glScissor((int) (x * mc.gameSettings.guiScale), (int) ((mc.displayHeight - (y + height)) * mc.gameSettings.guiScale), (int) (width * mc.gameSettings.guiScale), (int) (height * mc.gameSettings.guiScale));
    }
