...
public class ColorUtils {
    ...
    public static Color colorSwitch(Color firstColor, Color secondColor, float time, int index,
                                    long timePerIndex, double speed) {
        return colorSwitch(firstColor, secondColor, time, index, timePerIndex, speed, 255.0D);
    }

    public static Color colorSwitch(Color firstColor, Color secondColor, float time, int index,
                                    long timePerIndex, double speed, double alpha) {
    ...
    public static int getOverallColorFrom(int color1, int color2, float percentTo2) {
        ...
        final int finalGreen = (int) MathUtils.lerp(color1 >> 8 & 0xFF, color2 >> 8 & 0xFF, percentTo2);
        final int finalBlue = (int) MathUtils.lerp(color1 & 0xFF, color2 & 0xFF, percentTo2);
        final int finalAlpha = (int) MathUtils.lerp(color1 >> 24 & 0xFF, color2 >> 24 & 0xFF, percentTo2);
    ...
    public static int swapAlpha(int color, float alpha) {
        ...
        int g = color >> 8 & 0xFF;
        int b = color & 0xFF;
        return ColorUtils.getColor(r, g, b, (int) alpha);
    }
    ...
    public static int getHealthColor(EntityLivingBase player) {
        ...
        float healthPercentage = Math.max(0.0F, Math.min(health, maxHealth) / maxHealth);
        return Color.HSBtoRGB(healthPercentage / 3.0F, 0.75F, 1.0F) | 0xFF000000;
    }
    ...
    public static Color fade(Color color, int index, int count) {
        ...
        float brightness = Math.abs(((float) (System.currentTimeMillis() % 2000L) / 1000.0F
                + (float) index / (float) count * 2.0F) % 2.0F - 1.0F);
