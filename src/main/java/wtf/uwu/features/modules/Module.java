...
public abstract class Module implements InstanceAccess {
    ...
    public <M extends Module> boolean isEnabled(Class<M> module) {
        Module mod = UwU.INSTANCE.getModuleManager().getModule(module);
    ...
    private void enable() {
        ...
        try {
            UwU.INSTANCE.getNotificationManager().post(NotificationType.OKAY, "Module", getName() + EnumChatFormatting.GREEN + " enabled");
            onEnable();
            playClickSound(1.0F);
        } catch (Exception e) {
            handleException(e);
    ...
    private void disable() {
        UwU.INSTANCE.getEventManager().unregister(this);
        try {
            UwU.INSTANCE.getNotificationManager().post(NotificationType.WARNING, "Module", getName() + EnumChatFormatting.RED + " disabled");
            onDisable();
            playClickSound(0.8F);
        } catch (Exception e) {
            handleException(e);
    ...
    public <M extends Module> M getModule(Class<M> clazz) {
        return UwU.INSTANCE.getModuleManager().getModule(clazz);
    }
