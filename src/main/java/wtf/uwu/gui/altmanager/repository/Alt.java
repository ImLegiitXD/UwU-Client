...
public class Alt {
    ...
    public CompletableFuture<Session> logIn(boolean trippsol) {
        CompletableFuture<Session> sessionCompletableFuture = CompletableFuture.supplyAsync(() -> {
            ...
            if (!isLoggingIn() && !isLoginSuccessful()) {
                ...
                if (credential instanceof MicrosoftAltCredential) {
                    try {
                        ...
                        if (Alt.accountCheck(accessToken)) {
                            ...
                            UwU.INSTANCE.getNotificationManager().post(NotificationType.NOTIFY,"Logged in! " + Alt.this);

                        }
                    } catch (Throwable e) {
                        setLoginProperty(false);
                        setInvalid(true);
                        UwU.INSTANCE.getNotificationManager().post(NotificationType.NOTIFY,e.getClass().getName() + ':' + e.getMessage());
                    }
                } else {
                    session = new AltLoginThread(credential, new SessionUpdatingAltLoginListener() {
                        ...
                        public void onLoginFailed() {
                            setLoginProperty(false);
                            setInvalid(true);
                            UwU.INSTANCE.getNotificationManager().post(NotificationType.NOTIFY,"Invalid credentials!");
                        }
                    }).run();
                }
                ...
                if (System.currentTimeMillis() > lastTimeAlreadyLogged + 150) {
                    UwU.INSTANCE.getNotificationManager().post(NotificationType.NOTIFY,"Already trying logging in!");
                    this.lastTimeAlreadyLogged = System.currentTimeMillis();
                }
            } else if (isLoginSuccessful()) {
                if (System.currentTimeMillis() > lastTimeAlreadyLogged + 150) {
                    UwU.INSTANCE.getNotificationManager().post(NotificationType.NOTIFY,"Already logged in!");
                    this.lastTimeAlreadyLogged = System.currentTimeMillis();
                }
            }
        ...
        return sessionCompletableFuture.whenCompleteAsync((session, throwable) -> {
            if (throwable != null) {
                UwU.LOGGER.warn("An error occurred while logging in!", throwable);
                return;
            }
            ...
            if (isLoginSuccessful() && session != null && session.getProfile() != null && session.getProfile()
                ...
                try {
                    UwU.INSTANCE.getNotificationManager().post(NotificationType.OKAY, "Logged in! " + this);

                    repository.saveAlts();
                } catch (Throwable t) {
                    UwU.LOGGER.warn("An unexpected error occurred while loading Hypixel profile!", t);
                }
            }
        });
    }
    ...
    public static boolean accountCheck(String token) {
        ...
        try {
            ...
            if (!multiPlayerServerPrivilege.get("enabled").getAsBoolean()) {
                UwU.INSTANCE.getNotificationManager().post(NotificationType.NOTIFY,"Oops, this player don't have privilege to play online server.");
                return false;
            }
            ...
            if (attributes.has("banStatus")) {
                ...
                if (bannedScopes.has("MULTIPLAYER")) {
                    ...
                    if (!bannedScopes.has("expires") ||
                        ...
                        UwU.INSTANCE.getNotificationManager().post(NotificationType.NOTIFY,"Oops, this player got banned from mojang.");
                        return false;
                    }
                }
            }
        } catch (Throwable e) {
            UwU.INSTANCE.getNotificationManager().post(NotificationType.WARNING,"Failed to get player attributes.");
            e.printStackTrace();
        }
