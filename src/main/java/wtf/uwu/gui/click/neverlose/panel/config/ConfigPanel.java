...
package wtf.uwu.gui.click.neverlose.panel.config;
...
public class ConfigPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (configMap.keySet().stream().filter(ConfigRect::isSelected).findAny().orElse(null) == null) {
            if (!configMap.keySet().stream().toList().isEmpty()) {
                Objects.requireNonNull(configMap.keySet().stream().filter(configRect -> configRect.getConfig().getName().equals(UwU.INSTANCE.getConfigManager().getCurrentConfig())).findFirst().orElse(null)).setSelected(true);
            }
        }
        //update coordinate
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (MouseUtils.isHovered2(posX + 228,posY + 14,70, 22, mouseX, mouseY)) {
            UwU.INSTANCE.getConfigManager().saveConfig(new Config(text + ".json"));
            refresh();
        }
        for (ConfigRect configRect : configMap.keySet()) {
            ...
            if (mouseButton == 2 && configRect.isHovered(mouseX,mouseY)){
                UwU.INSTANCE.getConfigManager().loadConfig(configRect.getConfig());
            }
        }
    ...
    public void refresh(){
        configMap.clear();
        Arrays.stream(Objects.requireNonNull(UwU.INSTANCE.getMainDir().listFiles())).filter(file -> file.isFile() && file.getName().endsWith(".json")).forEach(file -> configMap.put(new ConfigRect(new Config(file.getName().replaceFirst(".json",""))),new Config(file.getName().replaceFirst(".json",""))));
